...
from Utils.ransac import find_bad_by_ransac
from Utils.removeTrend import removeTrend
from Utils.utils import _filter_design, _mat_iqr, _mat_quantile
...
class NoisyChannels:
    ...
    do_detrend : bool
        Whether or not low-frequency (<0.5 Hz) trends should be removed from the
    ...
    def __init__(
        ...
        *,
        ...
        self.bads_unusable = self.bad_by_manual + bads_by_nan_flat

        # Make a subset of the data containing only usable EEG channels
        self.usable_idx = np.isin(ch_names, self.bads_unusable, invert=True)
        try:
    ...
    def _get_filtered_data(self, lowcut=0.5, highcut=45,fir_design='firwin'):
        """Apply a [0.5 Hz - 45 Hz] bandpass filter to the EEG signal.

        Only applied if the sample rate is above 95 Hz to avoid violating the
        Nyquist theorem a few more.

        """
        if self.sample_rate <= 95:
            return self.raw_mne.copy()

        # Filters the data using a zero-phase FIR filter (same as MATLAB PREP)
        raw_filt=self.raw_mne.copy().filter(lowcut, highcut, fir_design=fir_design)

        return raw_filt

    def get_bads(self, verbose=False, as_dict=False, proportion_bads_aceptable=0.4):
        ...
        """
        if len(self.bads_unusable)<proportion_bads_aceptable*self.n_chans_original and self.bad_by_ransac is not None:
            bads = {
    ...
    def find_all_bads(
        ...
        if self.EEGData is not None:
            ...
            logger.warning(
                "No EEG data available to find bad channels. "
                "Please check that the provided MNE Raw object contains EEG channels."
            )
    ...
    def find_bad_by_deviation(self, deviation_threshold=5.0):
        ...
        if self.EEGData is not None:
            ...
            self._extra_info["bad_by_deviation"].update(
                {
                    "median_channel_amplitude": np.nan,
                    "channel_amplitude_sd": np.nan,
                    "robust_channel_deviations": np.full(self.n_chans_original, np.nan),
                }
            )

    def find_bad_by_hfnoise(self, HF_zscore_threshold=6.0):
        ...
        """
        MAD_TO_SD = 1.48  # Scales units of MAD to units of SD, assuming normality
        # Reference: https://stat.ethz.ch/R-manual/R-devel/library/stats/html/mad.html

        if self.EEGData is not None:
            
            if self.EEGFiltered is None:
                self.EEGFiltered = self._get_filtered_data().get_data(picks=self.ch_names_original[self.usable_idx])
            ...
            self._extra_info["bad_by_hf_noise"].update(
                {
                    "median_channel_noisiness": np.nan,
                    "channel_noisiness_sd": np.nan,
                    "hf_noise_zscores": np.full(self.n_chans_original, np.nan),
                }
            )
    ...
    def find_bad_by_correlation(
        ...
        if self.EEGData is not None:
            ...
            self._extra_info["bad_by_correlation"] = {
                "max_correlations": np.full((self.n_chans_original, self.n_chans_original), np.nan),
                "median_max_correlations": np.full(self.n_chans_original, np.nan),
                "bad_window_fractions": np.full(self.n_chans_original, np.nan),
            }
            self._extra_info["bad_by_dropout"] = {
                "dropouts": np.full((self.n_chans_original, self.n_chans_original), np.nan),
                "bad_window_fractions": np.full(self.n_chans_original, np.nan),
            }
            self._extra_info["bad_by_deviation"]["channel_amplitudes"] = np.full(
                (self.n_chans_original, self.n_chans_original), np.nan
            )
            self._extra_info["bad_by_hf_noise"]["noise_levels"] = np.full(
                (self.n_chans_original, self.n_chans_original), np.nan
            )
    ...
    def find_bad_by_ransac(
        ...
        if ch_correlations_usable is not None:
            ...
            self._extra_info["bad_by_ransac"] = {
                "ransac_correlations": np.full((self.n_chans_original, self.n_chans_original), np.nan),
                "bad_window_fractions": np.full(self.n_chans_original, np.nan),
            }