...
import pandas as pd
from numpy import random, concatenate, array_split
import torch
...
def cv_patients( patients_train: list,
    demograp_data, n_split=5,
    shuffle=True, random_state=0):
    """
    Genera particiones de pacientes para validación cruzada.

    La división se realiza a nivel de paciente, de modo que
    todas las muestras/ventanas pertenecientes a un paciente
    quedan en el mismo fold.
    """

    patients = array(patients_train)
    ...
    # 2. Crear folds
    # -----------------------------
    patient_folds = array_split(patients, n_split)
    ...
    for i in range(n_split):
        ...
        p_test = patient_folds[i].tolist()
        p_train = concatenate([patient_folds[j] for j in range(n_split) if j != i]).tolist()

        folds.append((p_train, p_test))

        # -----------------------------
        # 4. Convertir pacientes
        #    a índices de demograp_data
        # -----------------------------

        i_train = demograp_data[demograp_data["Id_Patient"].isin(p_train)].index.tolist()

        i_test = demograp_data[demograp_data["Id_Patient"].isin(p_test)].index.tolist()

        Indx.append((i_train, i_test))
...
def patient_stratify_split(demograp_data, train_size=0.8, shuffle=True):
    ...
    for i in [demograp_data[demograp_data["Id_Patient"]==i].index.to_list() for i in patients_val]:
        ind_val+=i
    return list(set(demograp_data["Id_Patient"].iloc[ind_train])),list(set(demograp_data["Id_Patient"].iloc[ind_val]))
############################################################
