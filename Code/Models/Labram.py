...
for h in hour_list:
    ...
    Raws += [mne.io.read_raw_fif(path_files+h+"/"+file, preload=True) for file in file_list]
...
parameters={"optimizer__lr":lr, "optimizer__weight_decay":weight_decay, "module__drop_prob":np.linspace(0.1, 0.4, 2),
    "module__num_layers": [ 8, 12], "module__num_heads": [4, 8], "module__patch_size": [256], "module__activation": [torch.nn.SiLU, torch.nn.ReLU],
    "module__ch_names": [channels], "module__n_times": [n_times], "module__n_chans":[n_chans], "module__n_outputs":[len(outputs)], "module__sfreq":[128]}
...
grid_clf=GridSearchCV(clf, parameters, n_iter=20, refit=True, cv=cv_patients(patients_train, n_split=3), scoring="roc_auc", verbose=1)
...
plt.title(f"Labram model")
plt.savefig(f"Labram model confusion matrix.png", dpi=300)
plt.show()
...
plt.grid(True)
plt.savefig(f"Labram model ROC curve.png", dpi=300)
plt.show()