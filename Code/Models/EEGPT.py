...
model = EEGPT(n_times=n_times, n_chans=n_chans, n_outputs=len(outputs), patch_size=128,patch_stride=32)
model.cuda()
...
parameters={"optimizer__lr":lr, "optimizer__weight_decay":weight_decay, "module__drop_prob":np.linspace(0.1, 0.4, 2), "module__embed_dim": [256, 512],
            "module_drop_prob": [0,0.1], "module__n_times": [n_times], "module__n_chans":[n_chans], "module__n_outputs":[len(outputs)]}
...
plt.savefig(f"EEGPT model confusion matrix.png", dpi=300)
plt.show()
...
plt.grid(True)
plt.savefig(f"EEGPT model ROC curve.png", dpi=300)
plt.show()