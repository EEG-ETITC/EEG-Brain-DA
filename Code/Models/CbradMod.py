...
model = CBraMod(n_times=n_times, n_chans=n_chans, n_outputs=len(outputs))
model.cuda()
...
parameters={"optimizer__lr":lr, "optimizer__weight_decay":weight_decay, "module__drop_prob":np.linspace(0.1, 0.4, 2), "module__n_layers": [8,12],
...
y_pred= grid_clf.predict(X_val[:,:,:-40])
...
plt.title(f"CBradmod model")
plt.savefig(f"CBradMod model confusion matrix.png", dpi=300)
plt.show()
...
plt.grid(True)
plt.savefig(f"CBradMod model ROC curve.png", dpi=300)
plt.show()
