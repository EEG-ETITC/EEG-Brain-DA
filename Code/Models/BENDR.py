...
path_demographic="/Users/byepesg/Library/CloudStorage/OneDrive-EscuelaTecnologicaInstitutoTecnicoCentral/JUAN DAVID LEAL CAMPUZANO's files - Proyecto_cerebro/Code/d/data_patients.csv"
...
path_files="/Users/byepesg/Library/CloudStorage/OneDrive-EscuelaTecnologicaInstitutoTecnicoCentral/JUAN DAVID LEAL CAMPUZANO's files - Proyecto_cerebro/Code/raw_tif/"
hours=[32,  36, 40,  44 ]
...
model = BENDR(n_times=n_times, n_chans=n_chans, n_outputs=len(outputs), contextualizer_hidden=2048)
model.cuda()
...
parameters={"optimizer__lr":lr, "optimizer__weight_decay":weight_decay, "module__transformer_layers": [6, 8], "module__transformer_heads": [6 ,8],
...
plt.savefig(f"BENDR model confusion matrix.png", dpi=300)
plt.show()
...
plt.grid(True)
plt.savefig(f"BENDR model ROC curve.png", dpi=300)
plt.show()